...
function median_magnitude(matrix)
    i = filter(x -> !isnan(x), matrix)
    if length(i) > 0
        magnitudes = abs.(i)
        median_index = argmin(abs.(magnitudes .- median(magnitudes)))
        return i[median_index]
    end
    return NaN
end
